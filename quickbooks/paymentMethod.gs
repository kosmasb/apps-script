function getPaymentMethods() {
  ...
  var values = [];
  for (var k = 0; k < 10; k++) {
    var url = "https://quickbooks.api.intuit.com/v3/company/[companyId]/query?query=select * from PaymentMethod STARTPOSITION "+ start + "&minorversion=59"
    var headers = {
      "headers":{
    ...
    var valuesInner = [];
    for (var i = 0; i < dataSet.QueryResponse.PaymentMethod.length; i++) {
      var record = dataSet.QueryResponse.PaymentMethod[i];
      ...
      valuesInner.push([name, active, type, domain, sparse, id, syncToken, createTime, lastUpdatedTime].map(safeValue).join(','));
    }
    for (var m = 0; m < valuesInner.length; m++) {
         values.push(valuesInner[m]);
       }
    start = start + 100;
    if (dataSet.QueryResponse.PaymentMethod.length < 100) {
