function getClasses() {
  ...
  var schema = {
    fields: [
      ...
      {name: 'sub_class', type: 'BOOLEAN', mode: 'NULLABLE', description: ''},
  ...
  var values = [];
  for (var k = 0; k < 10; k++) {
    var url = "https://quickbooks.api.intuit.com/v3/company/[companyId]/query?query=select * from Class STARTPOSITION "+ start + "&minorversion=59"
    var headers = {
      "headers":{
    ...
    var valuesInner = [];
    for (var i = 0; i < dataSet.QueryResponse.Class.length; i++) {
      var record = dataSet.QueryResponse.Class[i];
      var name = record.Name;
      var subClass = record.SubClass;
      ...
      valuesInner.push([name, subClass, fullyQualifiedName, active, domain, sparse, id, syncToken, createTime, lastUpdatedTime].map(safeValue).join(','));     
    }
    for (var m = 0; m < valuesInner.length; m++) {
         values.push(valuesInner[m]);
       }
    start = start + 100;
    if (dataSet.QueryResponse.Class.length < 100) {
